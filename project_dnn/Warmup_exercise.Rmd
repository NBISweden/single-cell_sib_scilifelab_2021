...
knitr::include_graphics("figures/08/Why_single_cell_sub.png")
```
...
The loaded object has no column names. Set the column names to the cell barcode. 
First check that they are all unique. What would you do if they were not (and 
...
```{r, message = FALSE, warning = FALSE}
library(scater)
mt <- rownames(sce)[grep("^Mt", rowData(sce)$gene.symbols)]
mt
...
In each case, the biological variance of a gene can be estimated as the difference between the total variance and the modelled technical variance. This value can be used to rank the genes, and to select a set of "highly variable" genes to use as the basis of further downstream analysis. Here, we select the top 1,000 genes based on the trend fit, and add these to the `metadata` of the `SingleCellExperiment` object. 
